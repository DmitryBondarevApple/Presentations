...
export const MBSlideContainer = ({ children, number, label, className }) => {
  return (
    <div className={cn(
      ...
      <div
        ...
        style={{ background: 'radial-gradient(circle at top right, hsl(152 60% 40%), transparent 70%)' }}
      />
      <div
        className="absolute bottom-0 left-0 w-60 h-60 opacity-[0.03] pointer-events-none"
        style={{ background: 'radial-gradient(circle at bottom left, hsl(152 40% 30%), transparent 70%)' }}
      />
      ...
      <div className="absolute bottom-4 left-6 md:left-16 lg:left-24 font-heading text-xs tracking-[0.18em] text-muted-foreground/40 pointer-events-none select-none z-10">
        СДЕЛАЙ<span className="text-accent/40"> КРАСИВО</span>
      </div>
    </div>
  );
};
