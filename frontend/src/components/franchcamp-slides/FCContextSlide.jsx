...
const FCContextSlide = () => {
  return (
    <FCSlideContainer number={2} label="Контекст">
      <h2 className="font-heading text-3xl sm:text-4xl md:text-5xl font-bold text-foreground mb-6" data-testid="fc-context-title">
      ...
      <div className="flex flex-col lg:flex-row gap-8 mb-6">
          ...
          <div className="space-y-4">
            {bullets.map((t, i) => (
              <div key={i} className="flex items-start gap-3">
                ...
                <span className="font-body text-lg md:text-xl text-foreground/80 leading-relaxed">{t}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="lg:w-80 bg-card rounded-lg border border-border p-6">
          <div className="grid grid-cols-2 gap-5">
            {stats.map((s, i) => (
              <div key={i}>
                <span className="font-heading text-3xl md:text-4xl font-bold text-accent">{s.n}</span>
      ...
      <div className="bg-card rounded-lg border-l-[3px] border-l-accent border border-border p-5" data-testid="fc-context-conclusion">
