...
const FCOfferSlide = () => {
  return (
    <FCSlideContainer number={8} label="Предложение">
      ...
      <p className="font-body text-xs md:text-xl text-muted-foreground mb-2 md:mb-8">
        Важен не только контент, но и способ монетизации и внедрения в сеть
      </p>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-2 md:gap-5 mb-2 md:mb-6">
        {offers.map((o, i) => (
          <div key={i} className="bg-card rounded-lg border-l-[3px] border-l-accent border border-border p-3 md:p-8" data-testid={`fc-offer-${i}`}>
            <span className="inline-block px-2 py-0.5 md:px-3 md:py-1.5 rounded bg-accent/10 text-accent text-[10px] md:text-sm font-bold tracking-wider mb-1 md:mb-4">
              {o.num}
            </span>
            <h3 className="font-heading text-sm md:text-2xl font-bold text-foreground mb-0.5 md:mb-3">{o.title}</h3>
            <p className="font-body text-[11px] md:text-lg text-muted-foreground leading-snug md:leading-relaxed">{o.desc}</p>
          </div>
        ))}
      </div>

      <div className="bg-card rounded-lg border border-border p-2 md:p-5">
        <p className="font-heading text-xs md:text-lg font-semibold text-foreground mb-1 md:mb-3">
          Что входит в состав программы:
        </p>
        <div className="flex flex-wrap gap-x-3 md:gap-x-6 gap-y-0.5 md:gap-y-3">
          {programItems.map((item, i) => (
            <div key={i} className="flex items-center gap-1 md:gap-2">
              ...
              <span className="font-body text-[11px] md:text-lg text-foreground/80">{item}</span>
            </div>
          ))}
        </div>
      </div>
