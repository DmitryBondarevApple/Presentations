...
import { SLIDES, COVER, LECTURER, fmtList, isDense, TOTAL } from "@/data/aiCompetitorsSlides";
...
const BP = 14;      // основной размер текста (как слайд 12)
...
const boxBorder = { borderTopWidth: 0.5, borderRightWidth: 0.5, borderBottomWidth: 0.5, borderLeftWidth: 3, borderTopColor: T.border, borderRightColor: T.border, borderBottomColor: T.border, borderLeftColor: T.accent };
const Note = ({ children, bs = BP }) => (
  <View style={{ backgroundColor: T.card, borderRadius: 4, padding: 11, ...boxBorder, borderTopColor: T.border, borderRightColor: T.border, borderBottomColor: T.border, borderLeftColor: T.accent, marginBottom: 9 }}>
  ...
  <View style={{ flexDirection: "row", flexWrap: "wrap", justifyContent: "space-between", marginBottom: 9 }}>
    {items.map((c, i) => (
      <View key={i} style={{ width: "32%", backgroundColor: T.card, borderRadius: 4, padding: 10, borderWidth: 0.5, borderColor: T.border }}>
        <Text style={{ fontSize: BP, fontWeight: 700, color: T.fg, lineHeight: 1.22, marginBottom: 6 }}>{c.name}</Text>
        ...
        <Text style={{ fontSize: BS, color: T.fg2, lineHeight: 1.34, marginBottom: 6 }}>{c.strong}</Text>
        ...
        <Text style={{ fontSize: BS, color: T.muted, lineHeight: 1.34 }}>{c.weak}</Text>
      </View>
    ))}
  </View>
);
...
const Pairs = ({ items }) => (
  <View style={{ marginBottom: 9 }}>
    {items.map((p, i) => (
      <View key={i} style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 9 }}>
        <View style={{ width: "48.5%", backgroundColor: T.card, borderRadius: 4, padding: 11, borderWidth: 0.5, borderColor: T.border }}>
          <Text style={{ fontSize: 8.5, fontWeight: 700, color: T.muted, textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 3 }}>Слабо</Text>
          <Text style={{ fontSize: BP, color: T.muted, lineHeight: 1.4 }}>{p.weak}</Text>
        </View>
        <View style={{ width: "48.5%", backgroundColor: T.card, borderRadius: 4, padding: 11, borderTopWidth: 0.5, borderRightWidth: 0.5, borderBottomWidth: 0.5, borderLeftWidth: 3, borderTopColor: T.border, borderRightColor: T.border, borderBottomColor: T.border, borderLeftColor: T.accent }}>
          ...
          <Text style={{ fontSize: BP, color: T.fg2, lineHeight: 1.4 }}>{p.strong}</Text>
        </View>
      </View>
    ))}
  </View>
...
const Prompt = ({ intro, paras }) => (
  <View style={{ backgroundColor: T.card, borderRadius: 4, padding: 16, borderTopWidth: 0.5, borderRightWidth: 0.5, borderBottomWidth: 0.5, borderLeftWidth: 3, borderTopColor: T.border, borderRightColor: T.border, borderBottomColor: T.border, borderLeftColor: T.accent, marginBottom: 9 }}>
    {intro && <Text style={{ fontSize: 10, fontWeight: 700, color: T.accent, textTransform: "uppercase", letterSpacing: 0.8, marginBottom: 9 }}>{intro}</Text>}
    {paras.map((p, i) => (
      <View key={i} style={{ flexDirection: "row", gap: 9, alignItems: "flex-start", marginBottom: 7 }}>
        <Text style={{ fontSize: BP, fontWeight: 700, color: T.accent, opacity: 0.7 }}>{String(i + 1).padStart(2, "0")}</Text>
        <Text style={{ fontSize: BP, color: T.fg2, lineHeight: 1.4, flex: 1 }}>{p}</Text>
      </View>
    ))}
  </View>
);
...
const SwotQuad = ({ title, items, pos }) => (
  <View style={{ width: "48.5%", marginBottom: 9, backgroundColor: T.card, borderRadius: 4, padding: 10,
    borderTopWidth: 2.5, borderRightWidth: 0.5, borderBottomWidth: 0.5, borderLeftWidth: 0.5,
    ...
    {fmtList(items).map((it, i) => (
      <View key={i} style={{ flexDirection: "row", gap: 5, alignItems: "flex-start", marginBottom: 3 }}>
        <View style={{ width: 3.5, height: 3.5, borderRadius: 2, backgroundColor: pos ? T.accent : T.neg, marginTop: 4.5 }} />
        <Text style={{ fontSize: BS, color: T.fg2, lineHeight: 1.32, flex: 1 }}>{it}</Text>
      </View>
    ))}
  </View>
);
...
const MapAxes = ({ x, y, points, hypothesis }) => (
  <View style={{ marginBottom: 9 }}>
    ...
    <View style={{ flexDirection: "row", flexWrap: "wrap", justifyContent: "space-between" }}>
      {points.map((p, i) => (
        <View key={i} style={{ width: "48.5%", marginBottom: 9, backgroundColor: T.card, borderRadius: 4, padding: 10, borderWidth: 0.5, borderColor: T.border }}>
          <Text style={{ fontSize: BP, fontWeight: 700, color: T.fg, lineHeight: 1.2, marginBottom: 2 }}>{p.label}</Text>
          <Text style={{ fontSize: BS, color: T.muted, lineHeight: 1.32 }}>{p.desc}</Text>
        </View>
      ))}
    </View>
    {hypothesis && <Note bs={BS}>{hypothesis}</Note>}
  </View>
);
...
const Actions = ({ items }) => (
  <View style={{ marginBottom: 9 }}>
    {items.map((a, i) => (
      <View key={i} style={{ flexDirection: "row", alignItems: "center", gap: 10, marginBottom: 8, backgroundColor: T.card, borderRadius: 4, padding: 11, borderWidth: 0.5, borderColor: T.border }}>
        <Text style={{ flex: 1, fontSize: BP, color: T.muted, lineHeight: 1.34 }}>{a.factor}</Text>
        <Text style={{ fontSize: 15, color: T.accent, fontWeight: 700 }}>→</Text>
        <Text style={{ flex: 1.15, fontSize: BP, color: T.fg2, fontWeight: 600, lineHeight: 1.34 }}>{a.action}</Text>
      </View>
    ))}
  </View>
);
  ...
  <View style={{ flexDirection: "row", flexWrap: "wrap", justifyContent: "space-between", marginBottom: 9 }}>
    {items.map((g, i) => (
      <View key={i} style={{ width: "32%", backgroundColor: T.card, borderRadius: 4, padding: 10,
        borderTopWidth: 2.5, borderRightWidth: 0.5, borderBottomWidth: 0.5, borderLeftWidth: 0.5,
        borderTopColor: T.accent, borderRightColor: T.border, borderBottomColor: T.border, borderLeftColor: T.border }}>
        <Text style={{ fontSize: BP, fontWeight: 700, color: T.fg, marginBottom: 5 }}>{g.title}</Text>
        {fmtList(g.items).map((it, j) => (
          <View key={j} style={{ flexDirection: "row", gap: 5, alignItems: "flex-start", marginBottom: 3 }}>
            <View style={{ width: 3.5, height: 3.5, borderRadius: 2, backgroundColor: T.accent, marginTop: 4.5 }} />
            <Text style={{ fontSize: BS, color: T.fg2, lineHeight: 1.32, flex: 1 }}>{it}</Text>
          </View>
        ))}
      </View>
...
const Formula = ({ text }) => (
  <View style={{ backgroundColor: T.accentBg, borderWidth: 0.5, borderColor: T.accent, borderRadius: 6, paddingVertical: 18, paddingHorizontal: 14, alignItems: "center", marginBottom: 9 }}>
