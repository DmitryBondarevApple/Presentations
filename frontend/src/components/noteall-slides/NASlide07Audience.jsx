...
const NASlide07Audience = () => (
  <NASlideContainer number={7} label="Для кого">
    <h2 className="font-heading text-lg sm:text-2xl md:text-5xl font-bold text-foreground mb-1 md:mb-4" data-testid="na-audience-title">
    ...
    <div className="grid grid-cols-2 gap-2 sm:gap-4 md:gap-8">
      {segments.map((s, i) => (
        <div key={i} className="bg-card rounded-lg border border-border p-2.5 sm:p-5 md:p-7" data-testid={`na-segment-${i}`}>
          ...
          <h3 className="font-heading text-[11px] sm:text-sm md:text-xl font-bold text-foreground mb-0.5 sm:mb-1 md:mb-3">{s.title}</h3>
          <p className="font-body text-[10px] sm:text-xs md:text-lg text-muted-foreground leading-snug sm:leading-relaxed">{s.desc}</p>
        </div>
      ))}
    </div>
  </NASlideContainer>
