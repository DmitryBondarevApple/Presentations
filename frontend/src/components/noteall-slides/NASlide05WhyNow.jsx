...
const NASlide05WhyNow = () => (
  <NASlideContainer number={5} label="Почему сейчас">
    <h2 className="font-heading text-xl sm:text-2xl md:text-5xl font-bold text-foreground mb-1 md:mb-4" data-testid="na-whynow-title">
      Рынок готов <span className="text-accent">покупать прикладной AI</span>
    </h2>
    <p className="font-body text-sm sm:text-base md:text-xl text-muted-foreground mb-2 sm:mb-5 md:mb-10 leading-snug sm:leading-relaxed max-w-6xl">
    ...
    <div className="flex flex-col sm:flex-row gap-2 sm:gap-4 md:gap-8 mb-2 sm:mb-4 md:mb-8">
      {markets.map((m, i) => (
        <div key={i} className="flex-1 bg-card rounded-lg border border-border p-2.5 sm:p-5 md:p-7" data-testid={`na-market-${i}`}>
          <span className="inline-block px-2 py-0.5 sm:px-3 sm:py-1 md:px-4 md:py-1.5 rounded bg-accent/10 text-accent text-[10px] sm:text-[10px] md:text-sm font-bold tracking-wider mb-1.5 sm:mb-3 md:mb-4">
            {m.title}
          </span>
          <span className="block font-heading text-base sm:text-xl md:text-4xl font-bold text-accent">{m.val}</span>
          <p className="font-body text-[10px] sm:text-[10px] md:text-base text-muted-foreground mt-1 sm:mt-2">{m.period}</p>
          <p className="font-body text-[10px] sm:text-[10px] md:text-sm text-muted-foreground/50 mt-0.5 sm:mt-1">
