...
const NASlide11GTM = () => (
  <NASlideContainer number={11} label="Go-to-Market">
    <h2 className="font-heading text-lg sm:text-2xl md:text-5xl font-bold text-foreground mb-1 md:mb-5" data-testid="na-gtm-title">
      ...
      {lines.map((l, i) => (
        <div key={i} className="flex flex-col sm:flex-row bg-card rounded-lg border border-border overflow-hidden" data-testid={`na-gtm-${i}`}>
          <div className="sm:w-52 md:w-72 bg-accent/10 p-2.5 sm:p-4 md:p-7 flex flex-row sm:flex-col items-center sm:items-start gap-1.5 sm:gap-2 md:gap-3 shrink-0">
            <span className="font-heading text-xs sm:text-sm md:text-xl font-bold text-accent">{l.num}</span>
            <h3 className="font-heading text-xs sm:text-sm md:text-xl font-bold text-foreground">{l.title}</h3>
          </div>
          <div className="p-2.5 sm:p-4 md:p-7 space-y-1 sm:space-y-2 md:space-y-3">
            {l.items.map((item, j) => (
              <div key={j} className="flex items-start gap-1.5 sm:gap-2">
                ...
                <span className="font-body text-[10px] sm:text-xs md:text-lg text-foreground/80 leading-snug sm:leading-relaxed">{item}</span>
              </div>
            ))}
          </div>
        </div>
