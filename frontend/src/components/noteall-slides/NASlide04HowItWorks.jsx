...
const NASlide04HowItWorks = () => (
  <NASlideContainer number={4} label="Как работает">
    <h2 className="font-heading text-lg sm:text-2xl md:text-5xl font-bold text-foreground mb-1 md:mb-4" data-testid="na-how-title">
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-2 sm:gap-3 md:gap-6">
      {steps.map((s, i) => (
        <div key={i} className="bg-card rounded-lg border-t-[3px] border-t-accent border border-border p-2.5 sm:p-4 md:p-7" data-testid={`na-step-${i}`}>
          <span className="font-heading text-[10px] sm:text-xs md:text-base font-bold text-accent/70">{s.num}</span>
          <h3 className="font-heading text-xs sm:text-sm md:text-2xl font-bold text-foreground mt-1 sm:mt-2 md:mt-4">{s.title}</h3>
          <p className="font-body text-[10px] sm:text-xs md:text-lg text-muted-foreground leading-snug sm:leading-relaxed mt-1 sm:mt-2 md:mt-4">{s.desc}</p>
        </div>
      ))}
    </div>
  </NASlideContainer>
