...
const NASlide09BusinessModel = () => (
  <NASlideContainer number={9} label="Бизнес-модель">
    <h2 className="font-heading text-2xl sm:text-4xl md:text-5xl font-bold text-foreground mb-1 md:mb-3" data-testid="na-bmodel-title">
    ...
    <div className="flex flex-col lg:flex-row gap-4 md:gap-6">
      <div className="flex-1 bg-card rounded-lg border-2 border-accent p-4 md:p-6">
        <span className="inline-block px-2 py-0.5 md:px-3 md:py-1 rounded bg-accent text-background text-[10px] md:text-xs font-bold tracking-wider mb-3 md:mb-4">СЕЙЧАС</span>
        <h3 className="font-heading text-lg md:text-2xl font-bold text-foreground mb-2 md:mb-3">Оплата за AI-вызовы</h3>
        <p className="font-body text-xs md:text-base text-muted-foreground leading-relaxed mb-3 md:mb-4">
        ...
        <div className="space-y-1 md:space-y-2">
          {["Прозрачная стоимость каждого запроса", "Нет месячных платежей на старте", "Пополнение баланса в любой момент", "Общий баланс для команды"].map((item, i) => (
            <div key={i} className="flex items-start gap-2">
              ...
              <span className="font-body text-xs md:text-sm text-foreground/80">{item}</span>
            </div>
          ))}
        </div>
      </div>
      <div className="flex-1 bg-card rounded-lg border border-border p-4 md:p-6">
        <span className="inline-block px-2 py-0.5 md:px-3 md:py-1 rounded bg-accent/10 text-accent text-[10px] md:text-xs font-bold tracking-wider mb-3 md:mb-4">ЧЕРЕЗ 2–3 МЕС.</span>
        <h3 className="font-heading text-lg md:text-2xl font-bold text-foreground mb-2 md:mb-3">Подписная модель</h3>
        <p className="font-body text-xs md:text-base text-muted-foreground leading-relaxed mb-3 md:mb-4">
        ...
        <div className="space-y-1 md:space-y-2">
          {["Фиксированный бюджет для бизнеса", "Расширенные командные функции", "Приоритетная обработка", "Корпоративные интеграции"].map((item, i) => (
            <div key={i} className="flex items-start gap-2">
              ...
              <span className="font-body text-xs md:text-sm text-muted-foreground">{item}</span>
            </div>
          ))}
        </div>
      </div>
