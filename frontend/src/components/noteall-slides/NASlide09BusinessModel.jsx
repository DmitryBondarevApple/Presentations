...
const NASlide09BusinessModel = () => (
  <NASlideContainer number={9} label="Бизнес-модель">
    <h2 className="font-heading text-xl sm:text-2xl md:text-5xl font-bold text-foreground mb-1 md:mb-4" data-testid="na-bmodel-title">
    ...
    <div className="flex flex-col lg:flex-row gap-2 sm:gap-4 md:gap-6">
      <div className="flex-1 bg-card rounded-lg border-2 border-accent p-3 sm:p-5 md:p-8">
        ...
        <p className="font-body text-xs sm:text-sm md:text-lg text-muted-foreground leading-snug sm:leading-relaxed mb-2 sm:mb-3 md:mb-5">
        ...
        <div className="space-y-1 sm:space-y-2 md:space-y-3">
          {["Прозрачная стоимость каждого запроса", "Нет месячных платежей на старте", "Пополнение баланса в любой момент", "Общий баланс для команды"].map((item, i) => (
            <div key={i} className="flex items-start gap-1.5 sm:gap-2">
              ...
              <span className="font-body text-xs sm:text-sm md:text-base text-foreground/80">{item}</span>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="space-y-1 sm:space-y-2 md:space-y-3">
          {["Фиксированный бюджет для бизнеса", "Расширенные командные функции", "Приоритетная обработка", "Корпоративные интеграции"].map((item, i) => (
            <div key={i} className="flex items-start gap-1.5 sm:gap-2">
              ...
              <span className="font-body text-xs sm:text-sm md:text-base text-muted-foreground">{item}</span>
            </div>
          ))}
        </div>
      </div>
