...
const NASlide10Stage = () => (
  <NASlideContainer number={10} label="Текущая стадия">
    ...
    <div className="flex flex-col sm:flex-row gap-4 md:gap-8 mb-4 md:mb-8">
      ...
      <div className="flex-1 bg-card rounded-lg border border-border p-5 md:p-8 text-center">
        <span className="font-heading text-2xl md:text-5xl font-bold text-foreground">AI-first</span>
        <p className="font-body text-xs md:text-lg text-muted-foreground mt-2 md:mt-4">Вся разработка с использованием AI-инструментов</p>
      </div>
      <div className="flex-1 bg-card rounded-lg border border-border p-5 md:p-8 text-center">
        <span className="font-heading text-2xl md:text-5xl font-bold text-foreground">10–20x</span>
        <p className="font-body text-xs md:text-lg text-muted-foreground mt-2 md:mt-4">Сокращение расходов на разработку</p>
      </div>
    </div>
