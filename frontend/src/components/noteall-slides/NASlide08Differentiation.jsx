...
const NASlide08Differentiation = () => (
  <NASlideContainer number={8} label="Отличие">
    <h2 className="font-heading text-xl sm:text-2xl md:text-5xl font-bold text-foreground mb-1 md:mb-5" data-testid="na-diff-title">
    ...
    <div className="grid grid-cols-2 gap-2 sm:gap-4 md:gap-8">
      {diffs.map((d, i) => (
        <div key={i} className="bg-card rounded-lg border-l-[3px] border-l-accent border border-border p-2.5 sm:p-5 md:p-7" data-testid={`na-diff-${i}`}>
          <h3 className="font-heading text-sm sm:text-base md:text-xl font-bold text-foreground mb-1 sm:mb-2 md:mb-3">{d.title}</h3>
          <p className="font-body text-xs sm:text-sm md:text-lg text-muted-foreground leading-snug sm:leading-relaxed">{d.desc}</p>
        </div>
      ))}
    </div>
  </NASlideContainer>
