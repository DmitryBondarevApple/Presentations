...
const S02 = ({ total }) => (
  <Page size={[PW, PH]} style={ps()}>
    ...
    <H>Почему это подходит <HA>для Точки</HA></H>
...
const S08 = ({ total }) => (
  <Page size={[PW, PH]} style={ps()}>
    ...
    <KV headers={["Сегмент клиентов Точки", "Почему Noteall релевантен"]} fs={9.5} lw="32%" rows={[
      ["Маркетплейс-селлеры", "Нужно фиксировать задачи по поставкам, маркетингу, карточкам и подрядчикам"],
...
const cases = [
  ...
  { num: 10, label: "Кейс 2 · Подрядчики", title: "Подрядчик, проектная компания, стройбригада",
  ...
  { num: 11, label: "Кейс 3 · Бухгалтерия", title: "Бухгалтерская компания",
  ...
  { num: 12, label: "Кейс 4 · Digital / ИТ", title: "Digital-агентство или ИТ-аутсорсинг",
  ...
  { num: 14, label: "Кейс 6 · Онлайн-образование", title: "Онлайн-школа, эксперт, наставник",
  ...
  <Page size={[PW, PH]} style={ps()}>
    ...
    <View style={{ flexDirection: "row", gap: 8, marginBottom: 8 }}>
      <Card>
        ...
        {["маркетплейс-селлеры", "проектные подрядчики и стройбригады", "бухгалтерские и юридические компании", "digital-агентства", "ИТ-аутсорсинг", "онлайн-образование и эксперты"].map((t, i) => <Li key={i}>{t}</Li>)}
      </Card>
      <Card accent>
...
const ecoRows = [
  ["маркетплейс-селлеры", "100 000", "2%", "2 000", "15 000 ₽", "15 млн ₽"],
...
const S21 = ({ imgBase, total }) => (
  <Page size={[PW, PH]} style={ps()}>
    ...
    <Text style={{ fontSize: 30, fontWeight: 700, color: T.fg, lineHeight: 1.2, marginBottom: 12 }}>Предлагаем совместную <Text style={{ color: T.accent }}>продуктовую сессию</Text> с Точкой</Text>
    <Text style={{ fontSize: 12, color: T.muted, marginBottom: 12 }}>На сессии:</Text>
    {["Подробно ознакомиться с интерфейсом и функционалом Noteall", "Выбрать приоритетные сегменты клиентов", "Обсудить необходимые сценарии обработки встреч", "Согласовать требования по данным и безопасности", "Подготовить пилотный оффер для клиентов"].map((s, i) => (
      <View key={i} style={{ flexDirection: "row", gap: 10, alignItems: "flex-start", marginBottom: 7 }}>
    ...
    <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginTop: 22, paddingTop: 16, borderTopWidth: 0.5, borderTopColor: T.border }}>
      <View style={{ flexDirection: "row", alignItems: "center", gap: 12 }}>
        <Image src={`${imgBase}/images/tochka/speaker.png`} style={{ width: 48, height: 48, borderRadius: 24, objectFit: "cover" }} />
        <View>
      ...
      <View style={{ flexDirection: "row", alignItems: "center", gap: 14 }}>
        ...
        <Image src={`${imgBase}/images/tochka/${T.tochka}`} style={{ width: 104, height: 14, objectFit: "contain" }} />
        <Text style={{ fontSize: 11, color: T.muted }}>noteall.ru</Text>
      </View>
    </View>
  </Page>
);
