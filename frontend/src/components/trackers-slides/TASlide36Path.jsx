...
const TASlide36Path = () => (
  <TASlideContainer number={36} label="Сдвиг">
    <TAH>Путь стал короче в разы</TAH>
    <TASub>MVP и коммерчески применимый продукт — разница сокращается</TASub>
    <div className="space-y-3 md:space-y-5 max-w-4xl">
      <div className="rounded-md p-4 md:p-6" style={{ backgroundColor: "#fafafa", border: "1px solid #e5e5e5" }}>
        ...
        <div className="flex items-center gap-2 md:gap-3 flex-wrap">
          {before.map((s, i) => (<div key={i} className="flex items-center gap-2 md:gap-3">
            <span className="font-body text-sm md:text-lg px-3 py-1.5 rounded" style={{ backgroundColor: "#f0f0f0", color: "#71717a" }}>{s}</span>
            {i < before.length - 1 && <span style={{ color: "#d4d4d8" }} className="text-lg">→</span>}
          </div>))}
        </div>
      </div>
      <div className="rounded-md p-4 md:p-6" style={{ border: "2px solid #0a0a0a" }}>
        ...
        <div className="flex items-center gap-3 md:gap-5 flex-wrap">
          {after.map((s, i) => (<div key={i} className="flex items-center gap-3 md:gap-5">
            <span className="font-body text-sm md:text-xl font-semibold px-4 py-2 rounded" style={{ backgroundColor: "#0a0a0a", color: "#ffffff" }}>{s}</span>
            {i < after.length - 1 && <span style={{ color: "#0a0a0a" }} className="text-xl">→</span>}
          </div>))}
        </div>
      </div>
    </div>
