...
const H2 = ({ t, a }) => (
  <h2 className="font-heading text-lg sm:text-2xl md:text-4xl lg:text-[2.5rem] font-bold text-foreground leading-tight mb-2 sm:mb-3 md:mb-4">
    {t} {a && <span className="text-accent">{a}</span>}
  </h2>
);

const Lead = ({ children }) => (
  <p className="font-body text-xs sm:text-base md:text-lg text-muted-foreground leading-snug sm:leading-relaxed max-w-5xl mb-2 sm:mb-3 md:mb-4">{children}</p>
);
...
const Callout = ({ title, text }) => (
  <div className="bg-card rounded-lg border border-border border-l-4 border-l-accent p-2.5 sm:p-4">
    ...
    <p className="font-body text-xs sm:text-sm md:text-base text-foreground/85 leading-snug">{text}</p>
  </div>
);

const Bullet = ({ children }) => (
  <div className="flex items-start gap-1.5 md:gap-2.5">
    <div className="w-1 h-1 md:w-1.5 md:h-1.5 rounded-full mt-1.5 md:mt-2 shrink-0 bg-accent" />
    <p className="font-body text-[11px] sm:text-sm md:text-base text-foreground/85 leading-snug">{children}</p>
  </div>
);
...
const Bullets = ({ eyebrow, items, cols }) => (
  <div>
    ...
    <div className={cn("grid gap-x-4 gap-y-1 sm:gap-y-1.5",
  ...
  <div className={cn("grid gap-2 sm:gap-3 md:gap-4",
    ...
    {items.map((c, i) => (
      <div key={i} className="bg-card rounded-lg border border-border border-t-2 border-t-accent p-2.5 sm:p-4">
        ...
        {c.title && <p className="font-heading text-sm sm:text-base md:text-lg font-bold text-foreground leading-snug mb-1">{c.title}</p>}
        {c.desc && <p className="font-body text-[11px] sm:text-sm md:text-[15px] text-muted-foreground leading-snug">{c.desc}</p>}
      </div>
    ))}
  </div>
);
    ...
    {items.map((p, i) => (
      <div key={i} className="grid grid-cols-1 md:grid-cols-2 gap-2 sm:gap-4 items-stretch">
        <div className="bg-card/60 rounded-lg border border-border p-2.5 sm:p-4">
          ...
          <p className="font-body text-[11px] sm:text-sm md:text-base text-muted-foreground leading-snug">{p.weak}</p>
        </div>
        <div className="bg-card rounded-lg border border-border border-l-4 border-l-accent p-2.5 sm:p-4">
          <p className="font-heading text-[10px] sm:text-[11px] font-bold text-accent uppercase tracking-wider mb-1">Сильнее</p>
          <p className="font-body text-[11px] sm:text-sm md:text-base text-foreground/90 leading-snug">{p.strong}</p>
        </div>
      </div>
    ))}
  </div>
    ...
    <div className="space-y-2 sm:space-y-3">
      {paras.map((p, i) => (
        <div key={i} className="flex items-start gap-2.5 sm:gap-3">
          ...
          <p className="font-body text-xs sm:text-sm md:text-[15px] text-foreground/85 leading-snug sm:leading-relaxed">{p}</p>
        </div>
      ))}
    </div>
  </div>
  ...
  <div className="space-y-2 sm:space-y-3">
    {items.map((a, i) => (
      <div key={i} className="grid grid-cols-1 md:grid-cols-[1fr_auto_1.1fr] gap-2 md:gap-3 items-center bg-card rounded-lg border border-border p-2.5 sm:p-3.5">
        <p className="font-body text-[11px] sm:text-sm md:text-base text-muted-foreground leading-snug">{a.factor}</p>
        <span className="hidden md:inline text-accent font-bold text-lg">→</span>
        <p className="font-body text-[11px] sm:text-sm md:text-base text-foreground/90 font-medium leading-snug">{a.action}</p>
      </div>
    ))}
  </div>
);
...
const Formula = ({ text }) => (
  <div className="bg-accent/10 border border-accent/40 rounded-lg px-4 sm:px-6 py-3 sm:py-5 text-center">
    <p className="font-heading text-sm sm:text-xl md:text-2xl font-bold text-accent leading-snug">{text}</p>
  </div>
);
...
export const AICStandardSlide = ({ slide }) => (
  <AICShell number={slide.n} label={slide.label}>
    ...
    <div className="space-y-2.5 sm:space-y-3 md:space-y-4 flex-1 flex flex-col">
