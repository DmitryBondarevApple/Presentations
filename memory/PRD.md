...
### Редизайн + фикс скролла (28.06.2026, v2)
- **Баг скролла исправлен**: глобальное правило `App.css` (html,body,#root `overflow:hidden`, для слайдов) блокировало прокрутку флайера. Фикс: `RmaatFlyer.jsx` в useEffect переопределяет `overflow:auto`/`height:auto` на этих элементах при монтировании и восстанавливает при размонтировании.
- **Премиум-редизайн** по дизайн-гайду (`/app/design_guidelines.json`): новая палитра navy `#0A1225`/`#121C36`/`#1A2748` + orange `#F97316`, accent `#FDBA74`. Шрифты Montserrat (заголовки) + Manrope (текст) — web через Google Fonts, PDF через jsDelivr fontsource (cyrillic 700/800/900 и 400/600).
...
### Рынок стартапов — правки слайда «Рабочая группа» (20.07.2026, v2)
- Убран kicker «Кто провёл исследование» на слайде 2 (web `SMSlideTeam.jsx`).
- Шрифты и фото участников увеличены на +25% (web + PDF `STeam`/`TeamMember`/`GroupHead`): web фото 68→85px, имя 1.05rem→1.3rem, роль 13→16px, заголовок группы 18→20px; PDF фото 46→58px, имя 12→15, роль 9→11, заголовок группы 13→16.
- Заменено фото Натальи Старостиной (`team/starostina.jpg`, новое 254КБ). Теперь все 8 участников с реальными фото.
- Проверено: web-скриншот слайда 2 (kicker убран, крупнее, новое фото), оба PDF-блоба (light+dark) генерируются без ошибок; высота правой колонки (~389px) укладывается в доступные ~471px.


### Рынок стартапов — оптимизация фото + ленивая генерация PDF (20.07.2026, v3)
- **Корень проблемы**: фото команды были по 1.7–2.2 МБ (PNG). Тяжёлые изображения тормозили веб и не успевали встраиваться в PDF (@react-pdf/renderer) — часть фото не попадала в PDF.
- **Фикс фото**: все 8 фото пережаты в квадрат 320×320 JPEG q82 (Pillow, top-crop под object-top) → 8–14 КБ каждое (~90 КБ всего вместо ~15 МБ). Расширения обновлены на `.jpg` в `SMSlideTeam.jsx` и `StartupMarketPdfGenerator.jsx`.
- **Ленивая генерация PDF**: раньше оба PDF (light+dark) генерились eager при каждом монтировании/перезагрузке. Теперь генерация запускается только по клику на кнопку «PDF» (`ensurePdfs` + `genStartedRef`), кэшируется в рамках сессии; в меню показывается «Генерируется PDF…». Генерация ~3.2 сек.
- Проверено: все 8 фото на веб грузятся (naturalWidth=320), PDF генерируется за 3.2с без ошибок.

### Рынок стартапов — правки слайда команды (20.07.2026, v4)
- Заменено фото Ханифы Тыркба (team/tyrkba.jpg, новое, 320×320 JPEG 9КБ).
- Должность Кристины Катеневой: «Аналитик» → «Менеджер по масштабированию активов» (web + PDF).

### Рынок стартапов — фикс фото Тыркба (20.07.2026, v5)
- Причина: использовал устаревший артефакт `Ханифа Тыркба _2.png`; актуальный загруженный файл — `Ханифа_Тыркба_4.png`. Теперь применён правильный (`team/tyrkba-v3.jpg`, 320×320 JPEG). Старые tyrkba.jpg/tyrkba-v2.jpg удалены.
